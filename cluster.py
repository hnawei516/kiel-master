...
from kiel.exc import BrokerConnectionError, NoBrokersError, NoLeaderError
...
class Cluster(object):
    ...
    def get_leader(self, topic, partition_id):
        ...
        """
        if not self.leaders or not self.leaders[topic] or not self.leaders[topic][partition_id]:
            raise NoLeaderError
    ...
    def get_metadata(self, topics=None):
        ...
        for conn in self.conns.values():
            try:
                ...
                print("get_metadata:{}".format(topics, response))
                break
            except (iostream.StreamClosedError, BrokerConnectionError):
                print("get_metadata conn error:{}".format(topics))
                continue
