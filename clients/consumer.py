...
from kiel.exc import NoOffsetsError, BrokerConnectionError, NoBrokersError, NoLeaderError
    ...
    def consume(self, topic, start=None):
        ...
        try:
            ...
            if topic not in self.allocation or not self.allocation[topic]:
                log.error("Consuming unknown topic %s, reloading metadata", topic)
        ...
        #     yield self.connect()
            ...
            log.error("NoLeaderError error for topic %s", topic)
            yield gen.sleep(10)
            yield self.connect()
